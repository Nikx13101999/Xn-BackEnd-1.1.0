...
const getChange1Y = async (IPrice) => {
  const end = new Date(); // Today's dates
...
async function getMaxShareTotal(data) {
  const result = {};

  data.forEach(({ Share, Quantity, PurchsePrice, PurchaseType }) => {
    const value = Quantity * PurchsePrice;
    if (!result[Share]) result[Share] = 0;
    result[Share] += PurchaseType === "Paid Purchase" ? value : -value;
  });

  return {
    MaxResultValue: Math.max(...Object.values(result)),
    Result: result,
  };
}

async function generateNumbersWithSum(X, Y) {
  if (Y <= 0 || X <= 0) return [];

  // Generate Y random positive numbers
  const randoms = Array.from({ length: Y }, () => Math.random());

  // Get the sum of randoms
  const sum = randoms.reduce((a, b) => a + b, 0);

  // Normalize them so that their sum equals X
  const result = randoms.map((r) => (r / sum) * X);

  // Optional: Round to 2 decimal places and adjust the last value to fix rounding error
  const rounded = result.map((r, i) =>
    i === Y - 1
      ? +(X - result.slice(0, -1).reduce((a, b) => a + b, 0)).toFixed(2)
      : +r.toFixed(2)
  );
...
async function QuantityFind(PurchaseQuentityArray) {

  const PurchaseQuentityArrayInsideFunction = PurchaseQuentityArray.filter(
    (item) => item.PurchaseType !== "Paid Sell"
  );
  const PurchaseQuantity = PurchaseQuentityArrayInsideFunction.reduce(
    (sum, item) => sum + item.Quantity,
    0
  );

  const SellQuentityArrayInsideFunction = PurchaseQuentityArray.filter(
    (item) => item.PurchaseType == "Paid Sell"
  );
  const SellQuantity = SellQuentityArrayInsideFunction.reduce(
    (sum, item) => sum + item.Quantity,
    0
  );

  var PurchasedShare = PurchaseQuantity - SellQuantity;
...
async function EmitFunction(now, hours, minutes, second) {
  ...
  for (var i = 0; i < Record.length; i++) {
    ...
    const Record2 = await PurchaseShare.find({
      CategoryId: Record[i]._id,
      createdAt: { $gte: fiveMinutesAgo }
    });

    const maxTotal = await getMaxShareTotal(Record2);

    const Record3 = await Share.find({ CategoryId: Record[i]._id });

    const numbers = await generateNumbersWithSum(
      maxTotal.MaxResultValue,
      Record3.length
    );
    ...
    for (var j = 0; j < Record3.length; j++) {
      ...
      var PushElementArray = await RecordArray[RecordArray.length - 1].value;

        var PushElement;
        var InvestmentElement;
        if (hours == 9 && minutes == 15) {
          var PushElement = PushElementArray;
          var InvestmentElement = Record3[j].Investment;
        } else {

          const Record4 = await PurchaseShare.find({
            Share: Record3[j]._id,
          });
  
          var PurchasedShare = await QuantityFind(Record4);

          if (Record2.length == 0 || maxTotal.MaxResultValue <= 0) {
            ...
            if(PurchasedShare == 0){
              ...
              var CurrentPriceArray = Record3[j].PriceData[0];
              var CurrentPrice = CurrentPriceArray[CurrentPriceArray.length - 1].value // 200
              
              var Percentage = Math.random() * 12        
              var RemainingPercentage = 100 - Percentage; 

              var PushElement = ( CurrentPrice * RemainingPercentage ) / ( 100 )
              
              var CurrentInvestment = CurrentPrice - PushElement;
              var CurrentTotleInvestment = CurrentInvestment * PurchasedShare

              var InvestmentElement = Record3[j].Investment + CurrentTotleInvestment;

            }

          } else {

            const maxKey = Object.entries(maxTotal.Result).reduce((max, current) =>
              current[1] > max[1] ? current : max
            )[0];
    
            var Change = 0;
    
            if (Record3[j]._id.toString() == maxKey) {
    
              var TotleChange = Record3[j].Investment + numbers[j] - maxTotal.MaxResultValue;
              var Change = TotleChange / PurchasedShare;

              var InvestmentElement = 0;
    
            } else {
    
              if (PurchasedShare > 0) {

                var TotleChange = ( numbers[j] + Record3[j].Investment );
                var Change = TotleChange / PurchasedShare
                var InvestmentElement = 0;

              } else {

                var Change = numbers[j];

                var InvestmentElement = Record3[j].Investment + numbers[j];

              }
    
            }
    
            var PushElement = Record3[j].PriceData[0][Record3[j].PriceData[0].length - 1].value + Change;
        ...
        Record3[j].PriceData[0] = RecordArray.filter((_, index) => index >= 1 && index <= RecordArray.length);
        ...
        if (hours == 9 && minutes == 15) {
          ...
          Record3[j].PriceData[1] = RecordArray1.filter((_, index) => index >= 1 && index <= RecordArray1.length);
          Record3[j].PriceData[2] = RecordArray2.filter((_, index) => index >= 1 && index <= RecordArray2.length);
          Record3[j].PriceData[3] = RecordArray3.filter((_, index) => index >= 1 && index <= RecordArray3.length);

        }

        Record3[j].Investment = InvestmentElement;
        Record3[j].save();
  ...
  console.log("Data Update Process Has Been Completed Successfully");
  ...
  for(var i=0;i<UserRecord.length;i++){
    ...
    var token = UserRecord[i].Token;
    var title = "Nx"
    var body = `Hello Good Morning ${UserRecord[i].Name}`

    // var response = await fetch(`${process.env.BackendBASE_URL}/SendFirebaseNotification`, {
    //   method: "POST",
...
async function fetchData(interval = 60000) {
  ...
  console.log(`Fetch Data Run At ${hours}:${minutes}:${second}`);
