...
const razorpay = new Razorpay({
  key_id: "rzp_live_hylVpa2VIPXaNi",
  key_secret: "fsuLWUFXCojzdNsvuj0xrFbr",
});
...
class class1 {
    ...
    try {
      if (
        ...
        if (!ShareExist) {
          ...
          var YearData = await getChange1Y(req.body.Price);

          var MonthDataAray = await GetPrevious1M(req.body.Price);
          var MonthData = YearData.slice(-MonthDataAray.length);

          var FiveDayData = YearData.slice(-7);

          var SinleDayData = await generate1Day(YearData[YearData.length - 1].value);
    ...
    try {
      ...
      if (
        ...
        if (ShareExist) {
          ...
          const Record = await Share.find({
            _id: new ObjectId(req.body.ShareId),
          });

          var CurrentValueArray = Record[0].PriceData[0]

          if(CurrentValueArray[CurrentValueArray.length - 1].value == req.body.PurchsePrice){
            var a = {
            ...
            await PurchaseShare.find({ _id: data._id }).deleteOne();

            var a = {
              message: "Share Not Purchase Due To Share Price Not Match",
              status: `${HTTP.SUCCESS}`,
            };
            res.status(HTTP.SUCCESS).json(a);
  ...
  static capturepayment = async (req, res) => {
    ...
    try {
      ...
      const capturedPayment = await razorpay.payments.capture(
        ...
        req.body.amount,
        {              
          account_id: "rzp_live_hylVpa2VIPXaNi",
        }
      );
  ...
  // static NotificationSend = async (req, res) => {
